...
async def lifespan(app: FastAPI):
    yield
    if get_session.session is not None:
        await get_session.session.close()


app = FastAPI(default_response_class=RowJSONResponse, openapi_url=None, lifespan=lifespan)
